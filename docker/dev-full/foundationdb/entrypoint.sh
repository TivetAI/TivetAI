...
function check_dependencies() {
	log "INFO" "Checking required commands..."
	local commands=("fdbcli" "timeout" "tee" "pgrep" "cp" "sleep" "trap" "exec")
	for cmd in "${commands[@]}"; do
		if ! command -v "$cmd" &>/dev/null; then
			log "ERROR" "Missing required command: $cmd"
			exit 1
...
function backup_existing_config() {
	if [[ -f "$FDB_CLUSTER_FILE" ]]; then
		log "INFO" "Backing up existing cluster config to $CONFIG_BACKUP"
		cp -p "$FDB_CLUSTER_FILE" "$CONFIG_BACKUP"
	fi
}
...
function restore_backup_config() {
	if [[ -f "$CONFIG_BACKUP" ]]; then
		log "INFO" "Restoring backup config"
		cp -p "$CONFIG_BACKUP" "$FDB_CLUSTER_FILE"
	fi
}
...
function on_exit() {
	local exit_code=$?
	log "INFO" "Cleanup triggered with exit code $exit_code."
	# Add cleanup logic here if needed, e.g.:
	# restore_backup_config
	# stop fdbcli sessions
}
...
function configure_networking() {
	log "INFO" "Configuring networking mode..."
	export FDB_NETWORKING_MODE=container
	log "INFO" "FDB_NETWORKING_MODE set to container"
}

function apply_extra_config() {
	if [[ -f /var/fdb/extra_config.sh ]]; then
		log "INFO" "Applying extra configuration from extra_config.sh..."
		# shellcheck source=/var/fdb/extra_config.sh
		source /var/fdb/extra_config.sh
	fi
}
